...
Page({
  ...
  submitOrder(e) {
    ...
    if (type == 2 && (!this.data.address || !this.data.address.address)) {
      wx.showToast({
        icon: 'error',
        title: '填写收货地址',
      })
    ...
    arrNew.forEach(item => {
      let pro = db.collection("order").add({
        data: {
          name: type == 2 ? this.data.address.userName : app.globalData.userInfo.nickName, // type 1上门自取，2送货上门
          address: type == 2 ? this.data.address.address : '',
          phone: type == 2 ? this.data.address.phone : '',
